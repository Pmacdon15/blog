...
import {
	...
	schemaUpdateParagraphSection,
	schemaUpdateTitleSection,
} from '@/zod/zod-schema'
...
type SafeParseResult =
	| ReturnType<(typeof schemaUpdateTitleSection)['safeParse']>
	| ReturnType<(typeof schemaUpdateImageSection)['safeParse']>
	| ReturnType<(typeof schemaUpdateParagraphSection)['safeParse']>
...
export async function deleteBlogSection(
	blogId: number,
	sectionId: number,
	sectionTypeId: number,
) {
	...
	if (sectionTypeId === 2) {
		const result =
			await sql`SELECT src FROM ImageSection WHERE id = ${sectionId}`
...
export async function deleteBlog(blogId: number) {
	...
	if (imageSections.length > 0) {
		const blobUrls = imageSections
			...
			.filter(
				(src): src is string =>
					!!src && src.includes('blob.vercel-storage.com'),
			)
